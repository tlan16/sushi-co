...
PageJs.prototype = Object.extend(new CRUDPageJs(), {
	...
	,_getResultRow: function(row, isTitle) {
		...
		tmp.row = new Element('span', {'class': 'row'})
			...
			.insert({'bottom': new Element(tmp.tag, {'class': 'description col-sm-5 col-xs-12' + ( tmp.isTitle === true ? 'hidden-xs' :'')}).insert({'bottom': new Element('i').update(row.description)}) })
			.insert({'bottom': new Element(tmp.tag, {'class': 'position col-sm-2 hidden-xs'}).update(row.position) })
