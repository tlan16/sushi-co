<?php
/**
 * This is the listing page for DefaultNutrition
 *
 * @package    Web
...
class ListController extends CRUDPageAbstract
{
	 ...
	 */
	public $menuItem = 'DefaultNutrition';
		...
		try
		{
			...
			foreach($serachCriteria as $field => $value)
			{
				...
				switch ($field)
				{
					...
					case 'df_nut.nutritions':
						{
							$nutritions = explode(',', trim($value));
							if(count($value) > 0)
							{
								$ps = array();
								$keys = array();
								foreach($nutritions as $index => $value){
									$key = md5($field . '_' . $index);
									$keys[] = ':' . $key;
									$ps[$key] = trim($value);
								}
								$where[] = 'df_nut.nutritionId in (' . implode(',', $keys) . ')';
								$params = array_merge($params, $ps);
							}
							break;
						}
					case 'df_nut.serveMeasurements':
						{
							$serveMeasurements = explode(',', trim($value));
							if(count($value) > 0)
							{
								$ps = array();
								$keys = array();
								foreach($serveMeasurements as $index => $value){
									$key = md5($field . '_' . $index);
									$keys[] = ':' . $key;
									$ps[$key] = trim($value);
								}
								$where[] = 'df_nut.serveMeasurementId in (' . implode(',', $keys) . ')';
								$params = array_merge($params, $ps);
							}
							break;
						}
